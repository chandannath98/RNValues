...
const Camera = ({route, navigation}) => {
  ...
  const {datasend, itemId} = route.params;
  ...
  const editUserProfile = async () => {
    ...
    datas.append('salesmenable_id', datasend.salesmenable_id);
    datas.append('userable_id', itemId);
    //datas.append('next_visit', Currentdate);
    datas.append('userable_type', 'user');
    // datas.append('interestedAt', SelectedInterested);
    datas.append('longitude', datasend.in_latitude);
    datas.append('latitude', datasend.in_longitude);
    datas.append('pincode', datasend.pincodeaddress);
    datas.append('distance_km', datasend.totaldistance);
    datas.append('distance_time', datasend.distancetime);
    ...
    await axios
      .post('https://rnsvalves.com/api/v1/sales/checkin', datas, {
        ...
        onUploadProgress: progressEvent => console.log("sdrs ", progressEvent.loaded),
        onDownloadProgress: progressEvent => console.log("sdrsafss ", progressEvent.loaded),
      })
      .then(response => {
        setuseloader(false);
