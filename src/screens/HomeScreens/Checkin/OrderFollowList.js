...
const OrderFollowList = ({route, navigation}) => {
  ...
  const handlecheckinputout = data =>
    dispatch(actions.handlecheckinputout({data, navigation, udid, ItemId}));
  ...
  const handleSubmit = async () => {
    ...
    if (coordinateValid && channeltypeValid && UserlatValid && UserlongValid) {
      const data = {
        meeting_time: 30,
        action: value,
        comment: Orderdetails,
